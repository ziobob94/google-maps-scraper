...
export class MapScraperClass {
	...
	apiKey: string
	baseUrl: string
	entitiesInstance: MongoWrapperClass | null;
	googleClient: GoogleClientClass | null
	...
	initDependencies() {
		this.googleClient = new GoogleClientClass();
		this.entitiesInstance = this.parent?.entitiesInstance || new MongoWrapperClass(this) || null
	}
	...
	async startMaps({
		...
		try {
			while (!trovato) {
				...
				if (query)
					params.query = query.replace(/[^a-zA-Z]+/g, " ").toLowerCase()
				...
				const response: any = await this.googleClient?.getPlace(params)

				if (response.data.results) {
				// Elabora la risposta qui
					const t = response.data.results

					await this.mapPlaceDetails(t)
					...
					console.log(
						`Ricerca per query: ${params.query}, risultati: ${t.length
						}, paginazione: ${nextPageToken ? "presente" : "nessuna"}`
					)
	...
	async mapPlaceDetails(places: any[]) {
		for (let i = 0; i < places.length; i++) {
			const place = places[i]
			const id = place.place_id
			const details = await this.googleClient?.getPlaceDetails(id)
			if (details?.data?.result) {
				places[i] = { ...place, ...details.data.result }
			}
		}
	}
	...
	async runMaps({
		...
		regioni.forEach((el: any) => {
			...
			const tempProvinceMap: any =
				this.mappaRegioniItaliane[normalizedRegionKey]
		...
		if (!skipProvinceFilter) {
			Object.keys(filteredRegions[state]).forEach(
				(regionKey: any, index: number) => {
					const provinceLocal = filteredRegions[state][regionKey]
					const provinceFilteredKeys = Object.keys(provinceLocal).filter(
						(key: any) => {
							return province
								.map((el: any) => (el = this.normalizeName(el)))
								.includes(this.normalizeName(key))
						}
					)

					Object.keys(provinceLocal).forEach((provinciaKey: any) => {
						if (!provinceFilteredKeys.includes(provinciaKey)) {
							delete filteredRegions[state][regionKey][provinciaKey]
						}

						if (
							!skipComuniFilter &&
							provinceFilteredKeys.includes(provinciaKey)
						) {
							let tempComuni =
								filteredRegions[state][regionKey][provinciaKey]?.comuni
							if (tempComuni?.length > 0) {
								let temp = new Set()
								tempComuni.forEach((elComune: string) => {
									if (comuni.length === 0) temp.add(elComune)
									else if (comuni.includes(this.normalizeName(elComune)))
										temp.add(elComune)
								})
								filteredRegions[state][regionKey][provinciaKey].comuni = [
									...temp,
		...
		for (let i = 0; i < regioniKeys.length; i++) {
			...
			for (let j = 0; j < provinceKeys.length; j++) {
				...
				if (level === "provincia") {
					...
					console.log(
						"Sto ottenendo dati per la regione: " +
						nomeRegione.toUpperCase() +
						" e provincia " +
						nomeProvincia.toUpperCase() +
						"\n"
					)
				...
				// Avvia la ricerca su Google Maps utilizzando startMaps() con la query specificata
					let byMaps: MapQueryResults = await this.startMaps({
						query: tempQuery,
						filter: false,
						mode,
					})
					...
					result[state][nomeRegione][nomeProvincia] =
						{} as ProvinciaComuneResult

					const comuniRoot: any =
						filteredRegions[state][nomeRegione][nomeProvincia].comuni

					// Ciclo per i comuni
					for (let k = 0; k < comuniRoot?.length; k++) {
						tt = []
						const nomeComune = comuniRoot[k]
						const resRoot: ProvinciaComuneResult = result[state][nomeRegione][
							nomeProvincia
						] as ProvinciaComuneResult
						...
						// Se la modalità è "mongo", trasforma e inserisce i dati in un database MongoDB
						if (writeResult && mode === "mongo") {
							const transformed = this.transformForMongo({
								data: result,
								level,
								stateKeyParam: state,
								regionKeyParam: nomeRegione,
								provinceKeyParam: nomeProvincia,
								cityKeyParam: nomeComune,
							})
		...
		// Se la modalità non è "mongo", scrive i dati in un file JSON
			if (mode === "mongo") {
				...
				writeResultOnJsonFile(
					await this.cleanExportData(result),
					level + "_clean_byMaps"
				)
			}
		}
	 ...
	 */
	async cleanExportData(
		data: ScrapingResultMap,
		fields: string[] = [],
		flatten = true
	): Promise<ScrapingResultMap | MapQueryResults> {
		fields = []
						...
						for (const provinciaKey in regione) {
							if (Object.prototype.hasOwnProperty.call(regione, provinciaKey)) {
								...
								if (Array.isArray(provincia)) {
									const cleanedProvincia: MapQueryResults = provincia.map(
										(item) => {
		...
		return flatten ? await this.flattenResult(cleanedData) : cleanedData
	}


	async flattenResult(data: ScrapingResultMap) {
		let result: MapQueryResults = []
		for (const stateKey in data) {
			if (Object.prototype.hasOwnProperty.call(data, stateKey)) {
				const state = data[stateKey]
				...
				for (const regioneKey in state) {
					if (Object.prototype.hasOwnProperty.call(state, regioneKey)) {
						...
						for (const provinciaKey in regione) {
							if (
								Object.prototype.hasOwnProperty.call(regione, provinciaKey)
							) {
								const provincia = regione[provinciaKey]

								// Filtriamo MapQueryResults se è un array
								if (Array.isArray(provincia)) {
									result = [...result, ...provincia];
								} else {
									if (!provincia.comuni) continue;
									Object.keys((provincia.comuni as any)).forEach((element: string) => {
										const comune = (provincia.comuni as any)[element];
										if (comune) result = [...result, ...comune];
									});
								}
							}
						}
		...
		// 	try {
		// 		const res = await this.entitiesInstance?.estateAgencyModel?.create(item);
		// 		if (res){
	...
	filterFields(
		...
		if (comune !== "") filteredItem.comune = comune
		return filteredItem
	}
	 ...
	 */
	filterComuni(
		comuni: ComuneResult | undefined,
		fields: string[]
	): ComuneResult | undefined {
		...
		for (const comuneKey in comuni) {
			if (Object.prototype.hasOwnProperty.call(comuni, comuneKey)) {
				...
				const filteredComune: MapQueryResults = comune.map((item) => {
					return this.filterFields(item, fields, comuneKey)
				})
...
//     const dataToReturn: ScrapingResultMap = {};
	...
	async getExportByMaps(fileName: string, clean = false, fields = [], flatten = false) {
		const data = await getExtraction(fileName)
		if (clean) return await this.cleanExportData(data, fields, flatten)
		return data
	}
